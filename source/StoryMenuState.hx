...
class StoryMenuState extends MusicBeatState
{
	...
	{	
		...
		for (i in 0...weekData.weekSongs.length)
		{
	...
	override function update(elapsed:Float)
	{
		...
		txtWeekTitle.text = weekData.weekNames[curWeek].toUpperCase();
		txtWeekTitle.x = FlxG.width - (txtWeekTitle.width + 10);

		// FlxG.watch.addQuick('font', scoreText.font);

		difficultySelectors.visible = weekData.weekUnlocked[curWeek];
	...
	function selectWeek()
	{
		if (weekData.weekUnlocked[curWeek])
		{
			//epic :)
			if (!stopspamming)
			{
			...
			PlayState.storyPlaylist = weekData.weekSongs[curWeek];
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		if (curWeek >= weekData.weekSongs.length)
			curWeek = 0;
		if (curWeek < 0)
			curWeek = weekData.weekSongs.length - 1;
		...
		for (item in grpWeekText.members)
		{
			...
			if (item.targetY == Std.int(0) && weekData.weekUnlocked[curWeek])
				item.alpha = 1;
			else
				item.alpha = 0.6;
			bullShit++;
...
typedef WeekStuff = {
	public var weekSongs:Array<Dynamic>;
