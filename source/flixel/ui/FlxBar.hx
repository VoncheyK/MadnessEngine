...
class FlxBar extends FlxSprite
{
	...
	/**
	 * Makes the top and mid of the bar colours equal to desiredTopColour and desiredMidColour, the bottom is determined when initializing the bar.
	 * @param desiredTopColour FlxColor for the one third of the bar to be set to.
	 * @param desiredMidColour FlxColor for the two third of the bar to be set to.
	 * @return This FlxBar object with the generated image for background rendering
	 */

	public function divideBarByThreeAndSetColours(desiredTopColour:FlxColor = FlxColor.GREEN, desiredMidColour:FlxColor = FlxColor.RED){
		var pixelData:BitmapData = this.graphic.bitmap;
		final ONETHIRD:Float = 1/3;
		final TWOTHIRD:Float = 2/3;
		final height:Int = pixelData.height;
		for (y in 0...height){
			for (x in 0...pixelData.width){
				//these are y's
				final oneThirdHeight:Float = height * ONETHIRD;
				final twoThirdHeight:Float = height * TWOTHIRD;
				(y <= oneThirdHeight) ? pixelData.setPixel32(x, y, desiredTopColour) : (y > oneThirdHeight && y <= twoThirdHeight ? pixelData.setPixel32(x, y, desiredMidColour): null);
			}
		}
	}

	/**
	 * Makes the top half of the bar set to the desired colour.
     * @param   desiredColour   FlxColor that the top half colour of the bar will be set to.
     * @return This FlxBar object with generated image for background rendering.
	*/
    public function setTopHalfColourTo(desiredColour:FlxColor = FlxColor.GREEN, ?desiredColourMidBar:FlxColor = FlxColor.RED, desiredSplitNumber:Int = 2):FlxBar
    {
        //get the bitmap data
        var pixelData:BitmapData = this.graphic.bitmap;
		...
		for (y in 0...pixelData.height)
			for (x in 0...pixelData.width)
				(y >= (pixelData.height / 2) && x >= (pixelData.height / 2) - 6) ? return this : pixelData.setPixel32(x, y, desiredColour); 
		return this;
    }
