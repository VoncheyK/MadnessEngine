...
class OptionsMenu extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			trace( FlxG.save.data[options[i][1]] + " / " + FlxG.save.data.downScroll);

			var optionText:Alphabet = new Alphabet(0, 20 + (i * 90), options[i][0], true, false);
			optionText.ID = i;
		...
		grpOptionsTexts.forEach(function(txt:Alphabet)
			{
				...
				{
					currentDescription = options[curSelected][2];
					descriptionText.text = currentDescription;
	...
	function updateOptions()
		{
			if (options[curSelected][3] is Bool)
				FlxG.save.data[options[curSelected][1]] = !FlxG.save.data[options[curSelected][1]];

			grpOptionsTexts.forEach(function(txt:Alphabet)
				{
						txt.text = options[txt.ID][0] + " " + FlxG.save.data[options[curSelected][1]];
				});
		}
